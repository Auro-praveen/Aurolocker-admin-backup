...
const MOAEAST1layout = (props) => {
  const locksMapping = {
    allLocks: lockers.MOAEAST1,

    seatNoA: ["L1", "L5", "S9", "S13"],
    seatNoB: ["L2", "L6", "QR", "S14"],
    seatNoC: ["L3", "L7", "S10", "M15"],
    seatNoD: ["XL4", "XL8","S11"],
    seatNoE: ["M12", "M16"],
  };
            ...
            <tr>
              {locksMapping.seatNoA.map((row, index) =>
                locksMapping.allLocks.indexOf(row) > -1 ? (
                  <td
                    rowSpan={row.includes("M") ? 2 : row.includes("XL") ? 3 : 1}
                    colSpan={row.includes("XL") || row.includes("L") ? 2 : 1}
                    className={
                      row.includes("QR")
                        ? "qr-scanner"
